...
def main():
    ...
    # 🔧  Completely override merge_mask_to_rgba – return uint8
    import types
    def robust_merge_mask_to_rgba(self, image, mask):
        H, W = image.shape[:2]
        m = np.ascontiguousarray(mask).reshape(H, W)
        if m.max() <= 1.0:
            m = (m * 255).astype(np.uint8)
        else:
            m = m.astype(np.uint8)
        m = m[..., np.newaxis]                        # (H, W, 1)
        rgba = np.concatenate([image[..., :3], m], axis=-1)
        return rgba.astype(np.uint8)

    inference.merge_mask_to_rgba = types.MethodType(robust_merge_mask_to_rgba, inference)
    ...
    for f in mask_files:
        m = imageio.imread(f)
        if m.ndim == 3:
            m = m[..., 0]         # single channel
        masks.append(m)

    # ---- Load the FULL‑SCENE pointmap (once for all objects) ----
    scene_hfer_path = os.path.join(args.image_dir, "scene_hfer.npy")
    if not os.path.exists(scene_hfer_path):
        raise FileNotFoundError(
            "scene_hfer.npy not found. Run convert_depth_to_hfer.py first on scene_full_depth.png"
        )

    scene_hfer = np.load(scene_hfer_path)          # shape (H, W, 3)
    print(f"Loaded scene pointmap: shape={scene_hfer.shape}, "
          f"Z in [{scene_hfer[...,2].min():.3f}, {scene_hfer[...,2].max():.3f}] m")

    # Provide this single pointmap to the pipeline
    if hasattr(inference, 'get_hfer'):
        inference.get_hfer(scene_hfer)              # set it once
    ...
    for i in range(len(masks)):
        ...
        # Note: no need to call inference.get_hfer() again – the same full‑scene
        # pointmap will be used for every object, and the mask selects the target.
