...
import sys, os, time, torch, numpy as np, argparse, imageio, glob, types
...
def compute_metric_scale_shift(hfer):
    """
    Returns:
        scale_vector: (3,) float32 array [s, s, s]
        shift: (1,3) float32 array
    """
    mask = np.any(hfer != 0, axis=-1)
    if not np.any(mask):
        return np.ones(3, dtype=np.float32), np.zeros((1,3), dtype=np.float32)
    pts = hfer[mask]
    centroid = pts.mean(axis=0, keepdims=True)  # (1,3)
    centered = pts - centroid
    max_dist = np.linalg.norm(centered, axis=1).max()
    scale_val = 1.0 / (max_dist + 1e-8)
    scale_vector = np.array([scale_val, scale_val, scale_val], dtype=np.float32)
    return scale_vector, centroid.astype(np.float32)

def inject_metric_scale(inference, hfer_list):
    """
    Monkey‑patch the preprocess_image method so that after the preprocessor
    returns a dict containing pointmap_scale/shift, we replace them with
    correct metric values derived from the per‑object pointmap (ignoring zeros).
    """
    original_preprocess = inference._pipeline.preprocess_image

    def patched_preprocess(self, image, preprocessor, pointmap=None):
        result = original_preprocess(image, preprocessor, pointmap=pointmap)
        # Only override if pointmap_scale is present (i.e., the ss_preprocessor call)
        if 'pointmap_scale' in result:
            if hasattr(self, '_hfer_list') and hasattr(self, '_current_obj_idx'):
                idx = self._current_obj_idx
                hfer = self._hfer_list[idx]
                scale_vec, shift = compute_metric_scale_shift(hfer)
                device = result['pointmap_scale'].device
                result['pointmap_scale'] = torch.tensor(scale_vec, device=device).unsqueeze(0)   # shape (1,3)
                result['pointmap_shift'] = torch.tensor(shift, device=device)                     # shape (1,3)
                print(f"  [CORRECTED] object {idx}: scale={scale_vec}, shift={shift}")
        return result

    inference._pipeline.preprocess_image = types.MethodType(patched_preprocess, inference._pipeline)
    inference._pipeline._hfer_list = hfer_list
    inference._pipeline._current_obj_idx = 0

# ---------- Main script ----------
def main():
    parser = argparse.ArgumentParser(description="3D Scene Inference with automatic metric scale")
    ...
    # Bypass the broken adaptive factor (still needed for per‑object pointmaps)
    ...
    # Manual mask loading (8‑bit single channel, as fixed earlier)
    ...
    if len(hfer_files) != len(masks):
        raise RuntimeError(f"Mismatch: {len(masks)} masks but {len(hfer_files)} per‑object pointmaps")
    hfers = [np.load(f) for f in hfer_files]
    print(f"Loaded {len(hfers)} per‑object pointmaps.")

    # Inject the metric scale override (will replace wrong normalization)
    inject_metric_scale(inference, hfers)
    ...
    for i in range(len(masks)):
        ...
        # Tell the preprocessor which object we are processing
        inference._pipeline._current_obj_idx = i

        # Give the pipeline the per‑object pointmap (as before)
        ...
        # Save pose parameters (now correct metric scale)
        pose_data = {
        ...
        # Apply the pose to get the truly metric rough mesh
        ...
        print(f"   💾 Saved truly metric mesh as object_{i}_rough_metric.glb")
    ...
    # Composition (video may still fail due to gsplat, ignore)
